...
ostream& operator <<(ostream& out, const Matrix<T>& mat){
    lock_guard<mutex> guard(mtx_OUT);
    if(!(mat.data.empty())) {
...
Matrix<T> Matrix<T>::operator +(Matrix<T>& mat){
    if(lines == mat.lines && cols == mat.cols){
        ...
        auto lambda = [](Matrix<T> mat, Matrix<T> mat2, size_t i, size_t j) {
            return mat.data[i][j] + mat2.data[i][j];
        };
        vector<vector<future<T>>> futures;
        futures.resize(lines);
        size_t lim = LIMIT;
        size_t I = 0;
        size_t J = 0;
        for(size_t i = 0; i < lines; i++){
            for(size_t j = 0; j < cols; j++){
                if(lim != 0) {
                    futures[i].push_back(async(launch::async, lambda, *this, mat, i, j));
                    lim -= 1;
                }
                else{
                    while(lim != LIMIT) {
                        tmp.data[I][J] = futures[I][J].get();
                        lim++;
                        J++;
                        if(J == cols){
                            J %= cols;
...
Matrix<T> Matrix<T>::operator *(Matrix<T>& mat){
    if (cols == mat.lines) {
        ...
        auto lambda = [&](Matrix<T> mat, Matrix<T> mat2, size_t i, size_t j) {
            T cnt = 0;
            for (int k = 0; k < mat2.lines; k++) {
                cnt += mat.data[i][k] * mat2.data[k][j];
            }
            return cnt;
        };
        vector<vector<future<T>>> futures;
...
Matrix<T> Matrix<T>::operator *(T value){
    if(!data.empty()) {
        ...
        auto lambda = [](Matrix<T> mat, T value, size_t i, size_t j) {
                return mat.data[i][j] * value;
        };
        vector<vector<future<T>>> futures;
        futures.resize(lines);
        size_t lim = LIMIT;
        ...
        for (size_t i = 0; i < lines; i++) {
            for (size_t j = 0; j < cols; j++) {
                if(lim != 0) {
                    futures[i].push_back(async(launch::async, lambda, *this, value, i, j));
                    lim -= 1;
                }
                else{
                    while(lim != LIMIT) {
...
Matrix<T> Matrix<T>::operator ~() const {
    if(isSquare() && det() != 0) {
        Matrix<T> tmp(lines, cols, 0);
        auto lambda = [](Matrix<T> mat, size_t i, size_t j) {
            return (mat.minor(i, j).det() * ((1 / mat.det()) * pow(-1, i + j)));
        };
        vector<vector<future<T>>> futures;
        futures.resize(lines);
        size_t lim = LIMIT;
        ...
        for (size_t i = 0; i < lines; i++) {
            for (size_t j = 0; j < cols; j++) {
                if(lim != 0) {
                    futures[i].push_back(async(launch::async, lambda, *this, i, j));
                    lim -= 1;
                }
                else{
                    while(lim != LIMIT) {
